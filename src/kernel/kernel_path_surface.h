      ...
      if (has_emission) {
        if (!blocked) {
          ...
          path_radiance_accum_light(
              kg, L, state, buffer, throughput * num_samples_inv, &L_light, shadow, num_samples_inv, lightgroups, is_lamp);
        }
        else {
...
#  ifdef __SHADOW_TRICKS__
  ...
  kernel_branched_path_surface_connect_light(kg, sd, emission_sd, state, buffer, throughput, 1.0f, L, all);
#  else
  /* sample illumination from lights to find path contribution */
  Ray light_ray ccl_optional_struct_init;
  BsdfEval L_light ccl_optional_struct_init;
...
#    endif
  ...
  if (has_emission) {
    if (!blocked) {
      /* accumulate */
      path_radiance_accum_light(kg, L, state, buffer, throughput, &L_light, shadow, 1.0f, lightgroups, is_lamp);
    }
    else {
