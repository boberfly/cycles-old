...
#  endif
  ...
  if (L && (closure_flag & SD_EMISSION)) {
    ...
    path_radiance_accum_emission(kg, L, state, buffer, *throughput, emission, 0);
  }
...
#  endif
      ...
      if (L && (closure_flag & SD_EMISSION)) {
        ...
        path_radiance_accum_emission(kg, L, state, buffer, tp, emission, 0);
      }
                        ...
                        bool heterogeneous)
{
  ...
  if (heterogeneous)
    return kernel_volume_integrate_heterogeneous_distance(kg, state, buffer, ray, sd, L, throughput);
  else
